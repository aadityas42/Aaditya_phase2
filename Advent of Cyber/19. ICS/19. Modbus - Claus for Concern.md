...
- We start off by learning what a SCADA(Supervisory Control and Data Aquisition) system is.
- We also learn a little about PLC(Programmable Logic Controller) which interface directly with hardware and respond in milliseconds in harsh environments
- Modbus is the protocol used for industrial devices to communicate with each other
- We use nmap for this challenge.
- We start off with initial reconnaisance to check what services are running on the system, one of which is a CCTV feed.
- We confirm this CCTV feed to be compromised since it shows us eggs instead of presents
- We use the following script to complete our reconnaisance
